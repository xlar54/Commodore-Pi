...
void* MemoryAllocate(u32 size) {	
	...
	if (FirstFreeAllocation == NULL) {
		...
		allocated = 0; // If we initialize at declaration, it is not guaranteed to be 0
	...
	LOG_DEBUGF("Allocated(%d) + size(%d) > 16384(%d) = %d\n", allocated, size, sizeof(Heap), allocated + size > 16384);
	if (allocated + size > 16384) {
		LOG("Platform: Out of memory 1! We should've had more heap space in platform.c.\n");
		return NULL;
	}
	
	if (FirstFreeAllocation == HEAP_END) {
		LOG("Platform: Out of memory 2! We should've had more allocations in platform.c.\n");
		return NULL;
	}
	...
	while (Current != HEAP_END) {
		if (Current->Next != HEAP_END) {
			...
			if ((u32)&Heap[sizeof(Heap)] - (u32)Current->Next - Current->Length >= size) {
				...
				LOG("Platform: Out of memory 3! We should've had more heap space in platform.c.\n");
